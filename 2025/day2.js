const input = `26803-38596,161-351,37-56,9945663-10044587,350019-413817,5252508299-5252534634,38145069-38162596,1747127-1881019,609816-640411,207466-230638,18904-25781,131637-190261,438347308-438525264,5124157617-5124298820,68670991-68710448,8282798062-8282867198,2942-5251,659813-676399,57-99,5857600742-5857691960,9898925025-9899040061,745821-835116,2056-2782,686588904-686792094,5487438-5622255,325224-347154,352-630,244657-315699,459409-500499,639-918,78943-106934,3260856-3442902,3-20,887467-1022885,975-1863,5897-13354,43667065-43786338
`
const idRanges = input.trim().split(",");
let totalInvalidIdsSum = 0;

for (let i = 0; i < idRanges.length; i++) {
    // Desestruturação para nomear diretamente o início e fim da faixa 🎯
    const [startId, endId] = idRanges[i].split("-").map(Number);
    
    for (let currentId = startId; currentId <= endId; currentId++) {
        const idString = currentId.toString();
        const maxPatternLength = Math.floor(idString.length / 2);
        
        let patternLength = 1;
        
        while (patternLength <= maxPatternLength) {
            if (idString.length % patternLength === 0) {

                const pattern = idString.substring(0, patternLength);

                const repeatCount = idString.length / patternLength;

                const reconstructedId = pattern.repeat(repeatCount);
                
                if (reconstructedId === idString) {
                    totalInvalidIdsSum += currentId; 
                    break; 
                }
            }
            patternLength++;
        }
    }
}

console.log(totalInvalidIdsSum);
