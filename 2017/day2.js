...
for (let i = 0; splitInput.length > i; i++) {
    ...
    let point = 0;
    while (line.length > point) {
        if (line[point].trim() !== "" && !isNaN(line[point])) {
            let currentValue = "";

            while (line.length > point && line[point].trim() !== "" && !isNaN(line[point])) {
                currentValue += line[point];
                point++;
            }
            point--;
...
for (let i = 0; splitInput.length > i; i++) {
    ...
    while (line.length > point) {
        if (line[point].trim() !== "" && !isNaN(line[point])) {
            ...
            point--;
            lineNumbers.push(parseInt(currentValue));
        }
        point++;
    }
    lineNumbers.sort((a, b) => b - a);
    let lineResult = 0;
    for (let j = 0; lineNumbers.length > j; j++) {
        for (let k = j + 1; lineNumbers.length > k; k++) {
            if (lineNumbers[j] % lineNumbers[k] === 0) {
                lineResult = lineNumbers[j] / lineNumbers[k];
                break;
            }
        }
        if (lineResult > 0) break;
    }
    total += lineResult;
}
